...
AC:
    ...
    Then, implement a feature which implements the acceptance criteria. Ensure that your test        
    passes, then upload your submission 
"""

class TestItemDone(TestCase):

      def setUp(self):
          self.client = Client()
          models.Item.objects.create(text="Setup dev environment")
          
      def test(self):
          response = self.client.get(reverse('item_list'))
          self.assertContains(response, '<div>Setup dev environment</div>')
          self.assertContains(response, '<button type="button">Done</button>')
